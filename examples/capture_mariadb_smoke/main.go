// Example: rds-mariadb smoke capture
//
// Exercises read-only MariaDB SDK endpoints (List/Get only — NO billable
// operations) so the capture middleware (NHN_SDK_CAPTURE_DIR, see
// nhncloud/internal/capture) mirrors response bodies to disk.
//
// The captured JSON files are consumed by Phase A.1.2 of the multi-engine
// clone plan to resolve UUIDs (flavor / subnet / parameter group / db
// version) used by the rds-mariadb-p99 scenario.
//
...
// (INI-ish format used by the CLI). The MariaDB app key may live under
// either the env var NHN_CLOUD_MARIADB_APPKEY or the credentials key
// `rds_app_key` (legacy CLI convention — shared across RDS engines).
//
// Run:
//
//	NHN_SDK_CAPTURE_DIR=$PWD/tests/captured-responses/rds-mariadb \
//	    go run ./examples/capture_mariadb_smoke
package main
...
func resolveCreds() (region, appKey, accessKey, secretKey string) {
	...
	appKey = os.Getenv("NHN_CLOUD_MARIADB_APPKEY")
	if appKey == "" {
		appKey = os.Getenv("NHN_MARIADB_APP_KEY")
	}
	if appKey == "" {
		// CLI convention: rds_mariadb_app_key is MariaDB-specific; falls back
		// to rds_app_key (which is MySQL-only despite the generic name).
		if v := file["rds_mariadb_app_key"]; v != "" {
			appKey = v
		} else if v := file["mariadb_app_key"]; v != "" {
			appKey = v
...
func main() {
	...
	if appKey == "" {
		missing = append(missing, "mariadb_app_key (NHN_CLOUD_MARIADB_APPKEY or rds_app_key)")
	}
	if accessKey == "" {
	...
	if firstInstanceID != "" {
		...
		// MariaDB SDK does not expose GetBackupInfo (MySQL-specific surface);
		// per-instance backup details are only reachable via ListBackups.
	} else {
